...
Both HTTP and Streamable HTTP transports support custom SSL configuration, which is useful for connecting to servers using internal Certificate Authorities or requiring specific TLS settings:
...
http_client = MCPClient.create_client(
  mcp_server_configs: [
    MCPClient.http_config(
      base_url: 'https://internal-mcp-server.company.com',
      ssl: {
...
# Use with Streamable HTTP transport
streamable_client = MCPClient.create_client(
  mcp_server_configs: [
    MCPClient.streamable_http_config(
      base_url: 'https://internal-mcp-server.company.com/mcp',
      ssl: {
        cert_store: cert_store,
        verify: true
      }
